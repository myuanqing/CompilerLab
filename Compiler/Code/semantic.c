...
void func_fundec(Node* fundec, Type type, int tag){
	...
	op->kind = NAME;
	op->u.name = (char*)(symbol+*(int*)&(fundec->sons->value));
	ic->u.label = op;
	insert_ir(ic);
...
void func_vardec_in_fundec(Node* vardec, ParaList tail, Type type){
	if(vardec->sons->siblings!=NULL){
		...
		//printf("%d\n",tp->u.array.size);
		tp->u.array.elem = type;
		func_vardec_in_fundec(vardec->sons, tail, tp);
	}
	else{
		Symbol_Entry sy_en = (Symbol_Entry)malloc(sizeof(struct Symbol_Entry_));
		Symbol_Entry sy_in_table = search_symbol_entry((char*)(symbol+*(int*)&(vardec->sons->value)) , cur_field);
...
void func_dec_in_compst(Node* dec, Type type){
	...
	if(dec->sons->siblings != NULL){
		...
		InterCode ic1 = (InterCode)malloc(sizeof(InterCode_));
		ic1->kind = ASSIGN;
		ic1->u.assign.left = place_f;
		ic1->u.assign.right = copy_operand(t1);
		insert_ir(ic1);
	}
...
Type func_vardec_in_compst(Node* vardec, Type type){
	...
	if(vardec->sons->siblings != NULL){
		...
		if((type->kind == STRUCTURE) || (type->kind == ARRAY)){
			Operand v1 = (Operand)malloc(sizeof(Operand_));	
			v1->kind = VARIABLE;
			v1->u.var_no = sy_en->no;		
			InterCode ic = (InterCode)malloc(sizeof(InterCode_));
			ic->kind = DEC;	
			ic->u.dec.v = v1;
			ic->u.dec.len = size_of_type(type);
			insert_ir(ic);
			sy_en->no = -sy_en->no;
		}
		if(irflag == 0){
...
int size_of_type(Type type){
	...
	case ARRAY:{
		return type->u.array.size * size_of_type(type->u.array.elem);
	}break;
	case STRUCTURE:{
		FieldList fl = type->u.structure;
		int count = 0;
		while(fl!=NULL){
			count += size_of_type(fl->type);
			fl = fl->tail;
		}
		return count;
	}break;
...
void func_stmt(Node* stmt){
	if(strcmp(stmt->sons->name, "Exp")==0){
		...
		Operand t2 = copy_operand(t1);
		InterCode ic = new_return_intercode(t2);
		insert_ir(ic);
		irflag = temp_irflag;
		...
		InterCode ic1 = new_label_intercode(lb1);
		insert_ir(ic1);
		...
		InterCode ic2 = new_label_intercode(lb2_f);
		insert_ir(ic2);	
		func_stmt(stmt->sons->siblings->siblings->siblings->siblings);
		
		Operand lb1_f = copy_operand(lb1);
		InterCode ic3 = new_goto_intercode(lb1_f);
		insert_ir(ic3);

		Operand lb3_f = copy_operand(lb3);
		InterCode ic4 = new_label_intercode(lb3_f);
		insert_ir(ic4);
		
	}
	else if(strcmp(stmt->sons->name,"IF")==0){
		...
		Operand lb1_f = copy_operand(lb1);
		InterCode ic1 = new_label_intercode(lb1_f);
		insert_ir(ic1);
		func_stmt(stmt1);
		if(stmt1->siblings != NULL){
			Node* stmt2 = stmt1->siblings->siblings;
			Operand lb3 = new_label();
			InterCode ic2 = new_goto_intercode(lb3);
			insert_ir(ic2);
			Operand lb2_f = copy_operand(lb2);
			InterCode ic3 = new_label_intercode(lb2_f);
			insert_ir(ic3);
			func_stmt(stmt2);	
			Operand lb3_f = copy_operand(lb3);
			InterCode ic4 = new_label_intercode(lb3_f);
...
Type func_exp(Node* exp, char* c){
	if(strcmp(exp->sons->name, "INT") == 0){
		*c = 'l';
		int value = *(int*)&(exp->sons->value);
		if((place != NULL) && (irflag == 1)){
			Operand int_op = new_int_constant(value);
			InterCode ic = new_assign_intercode(place, int_op);
			insert_ir(ic);
		}
		return TYPE_INT;
	}
		...
		*c = 'l';
		double value = *(double*)&(exp->sons->value);
		if((place != NULL) && (irflag == 1)){
			Operand float_op = (Operand)malloc(sizeof(Operand_));
			float_op->kind = FLOAT_CONSTANT;
			float_op->u.float_value = value;
			InterCode ic = new_assign_intercode(place, float_op);
			insert_ir(ic); 
		}
		return TYPE_FLOAT;
	}
		...
		if(exp->sons->siblings == NULL){
			...
			if(sy_en == NULL){
				...
				*c = 'r';
				Operand variable = get_variable(sy_en->no);
				
				if(irflag == 1){
					if(place!=NULL){
						InterCode ic = new_assign_intercode(place, variable);			
						insert_ir(ic);
					}
				}
				else{
			...
			if(sy_en == NULL){
				...
				*c = 'l';
				...
				if(strcmp((char*)(symbol + *(int*)&(exp->sons->value) ),"read")==0){
					InterCode ic = (InterCode)malloc(sizeof(InterCode_));
					ic->kind = READL;
					ic->u.label = place;			
					insert_ir(ic);
					...
					Operand name = (Operand)malloc(sizeof(Operand_));
					name->kind = NAME;
					name->u.name = (char*)(symbol + *(int*)&(exp->sons->value) );
					ic->u.assign.right = name;
					insert_ir(ic);
				}
			...
			if(sy_en == NULL){
				...
				*c = 'l';
				...
				if(strcmp((char*)(symbol + *(int*)&(exp->sons->value) ), "write")==0){
		...
		*c = 'l';
		...
		Operand place_f = place;
		Type tp;
		if( place_f!=NULL || called == 1){
			Operand op1 = new_temp();
			Operand op1_f = copy_operand(op1);
			place = op1;
			int temp_irflag = irflag;
			irflag = 1;
			tp = func_exp(exp->sons->siblings, &c1);
			irflag = temp_irflag;
			InterCode ic = (InterCode)malloc(sizeof(InterCode_));
			ic->kind = SUB;
			ic->u.binop.result = place_f;
			ic->u.binop.op2 = op1_f;
		...
		*c = 'l';
		...
		if(place_f != NULL || called ==1){
			if(called == 0){
				not_called_before(place_f);
			}
			Operand lb1 = label_true;
			Operand lb2 = label_false;
			label_false = lb1;
			label_true = lb2;
			called = 1;
			tp = func_exp(exp->sons->siblings, &c1);
			called = temp_called;
			if(called == 0){
				not_called_after(lb1,lb2,place_f);
			}
		}
		else{
			tp = func_exp(exp->sons->siblings, &c1);
	...
	else if(  strcmp(exp->sons->siblings->name, "PLUS") == 0 ){
		*c = 'l';
		char c1,c2;
		Type tp1,tp2;
		Operand place_f = place;
		if(place_f != NULL||called==1){		
			...
			place = op2;
			Operand op2_f = copy_operand(op2);
			...
			ic->kind = ADD;
			ic->u.binop.result = place_f;
			ic->u.binop.op1 = op1_f;
			ic->u.binop.op2 = op2_f;
			insert_ir(ic);
		}
		else{
			tp1 = func_exp(exp->sons, &c1);
	...
	else if(  strcmp(exp->sons->siblings->name, "MINUS") == 0 ){
		*c = 'l';
		char c1,c2;
		Type tp1,tp2;
		Operand place_f = place;
	...
	else if (  strcmp(exp->sons->siblings->name, "MUL") == 0 ){
		*c = 'l';
		char c1,c2;
		Type tp1,tp2;
		Operand place_f = place;
	...
	else if(  strcmp(exp->sons->siblings->name, "DIV") == 0 ){
		*c = 'l';
		char c1,c2;
		Type tp1,tp2;
		Operand place_f = place;
	...
	else if (  strcmp(exp->sons->siblings->name, "RELOP") == 0 ){
		*c = 'l';
		char c1,c2;
		Type tp1,tp2;
		Operand place_f = place;
		...
		if(place_f != NULL || called == 1){
			...
			ic1->kind = COND_GOTO;
			Operand t1_f = copy_operand(t1);
			Operand t2_f = copy_operand(t2);
			ic1->u.condop.t1 = t1_f;
			ic1->u.condop.t2 = t2_f;
			ic1->u.condop.lb = lb1;
			ic1->u.condop.op = (char*)(symbol+*(int*)&exp->sons->siblings->value);
			insert_ir(ic1);

			InterCode ic2 = new_goto_intercode(lb2);
			insert_ir(ic2);
	...
	else if(  strcmp(exp->sons->siblings->name, "AND") == 0 ){
		*c = 'l';
		char c1,c2;
		Type tp1,tp2;
		Operand place_f = place;
		...
		if(place_f != NULL||called == 1){
			...
			Operand nlb_f = copy_operand(nlb);
			InterCode ic = new_label_intercode(nlb_f);
			insert_ir(ic);
	
			label_true = lb1;
			label_false = copy_operand(lb2);
			called = 1;
			tp2 = func_exp(exp->sons->siblings->siblings, &c2);
			called = temp_called;
	...
	else if(  strcmp(exp->sons->siblings->name, "OR") == 0 ) {
		*c = 'l';
		char c1,c2;
		Type tp1,tp2;
		Operand place_f = place;
		...
		if(tp1 == NULL){
			...
			while(fls != NULL){
				if(strcmp(fls->name,(char*)(symbol+ *(int*)&(exp->sons->siblings->siblings->value)))==0){
					Operand bias = new_int_constant(length);
					Operand t1 = new_temp();
					InterCode ic = (InterCode)malloc(sizeof(InterCode_));
					ic->kind = ADD;
					...
					if(irflag == 1){
						InterCode ic1 = new_assign_intercode(place_f,t1_f);
						insert_ir(ic1);
					}
					else{
						place = t1_f;
		...
		*c = 'l';
		...
		Operand variable = place;
		
		Operand v_f;
	 	if(variable != NULL)
			v_f = copy_operand(variable);
		else 
			v_f = NULL;
		irflag = temp_irflag;
		...
		ir->kind = ASSIGN;
		ir->u.assign.left = variable;
		ir->u.assign.right = copy_operand(op);
		insert_ir(ir);
		irflag = temp_irflag;		
		
		//printf("here\n");
		if(place_f != NULL){
			InterCode ir1 = new_assign_intercode(place_f, v_f);
			insert_ir(ir1);
		}
		int t = type_judge(tp1,tp2);
		if(t == 0){
		...
		*c = 'r';
		...
		Type p = tp1->u.array.elem;
		int sz = 1;
		
		while((p != TYPE_INT) && (p != TYPE_FLOAT)){
			sz *= p->u.array.size;
			p = p->u.array.elem;
		}	
		
		Operand w = new_int_constant(sz*4);
		InterCode ic = (InterCode)malloc(sizeof(InterCode_));
		ic->kind = MUL;
		Operand t2 = new_temp(); 
		ic->u.binop.result = t2;
		ic->u.binop.op1 = copy_operand(t1);
		ic->u.binop.op2 = w;
		insert_ir(ic);
		...
		InterCode ic1 = (InterCode)malloc(sizeof(InterCode_));
		ic1->kind = ADD;
		ic1->u.binop.result = t3;
		ic1->u.binop.op1 = addr;
		ic1->u.binop.op2 = copy_operand(t2);
		insert_ir(ic1);			
		
		Operand t4 = copy_operand(t3);
		place = t4;		
		//place = t3;	
	
		
		if(tp1->u.array.elem == TYPE_INT || tp1->u.array.elem == TYPE_FLOAT){
			...
			if(irflag == 0){
				...
				InterCode ic2 = new_assign_intercode(place_f, t4);
				insert_ir(ic2);
			}
...
int type_judge(Type tp1, Type tp2){
	...
	if(tp1 == tp2){
		...
		if(tp1->kind!=tp2->kind){
			...
			//}
			//else 
			return type_judge(tp1->u.array.elem, tp2->u.array.elem);
		}
